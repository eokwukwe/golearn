...
func Login(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		json.NewEncoder(w).Encode(models.NewErrorResponse("Invalid credentials", nil))
		return
	}
	...
	if err != nil {
		...
		json.NewEncoder(w).Encode(models.NewErrorResponse("Failed to generate token", err))
		return
	}
	...
	if err != nil {
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusInternalServerError)
		json.NewEncoder(w).Encode(models.NewErrorResponse("Failed to create session", err))
		return
	}
	...
	if err != nil {
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusInternalServerError)
		json.NewEncoder(w).Encode(models.NewErrorResponse("Failed to get session ID", err))
		return
	}
	...
	json.NewEncoder(w).Encode(models.NewSuccessResponse("Login successful", response))
}
